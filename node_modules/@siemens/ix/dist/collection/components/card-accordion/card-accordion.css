...
:host .expand-action:hover {
  background: var(--theme-color-component-1--hover);
}
:host .expand-action:active {
  background: var(--theme-color-component-1--active);
}
:host .expand-action:not(.disabled):not(:disabled):focus-visible {
  outline: 1px solid var(--theme-color-focus-bdr);
  outline-offset: var(--theme-btn--focus--outline-offset);
}
:host .expand-action::before {
  content: "";
  position: absolute;
  width: 100%;
  height: 100%;
...
:host .expand-action:not(.show),
:host .expand-action:not(.show)::before {
...
:host .expand-content.show {
  grid-template-rows: 14.75rem;
  opacity: 1;
}
:host .expand-content-inner {
  position: relative;
  height: 100%;
  overflow: hidden;
}
:host .expand-content.show .expand-content-body {
  display: none;
  overflow: hidden;
